...
-T1_XML_2_FBX.exe: 
 ...
 All information must be contained into an .xml file, see doc.pdf and the examples provided. 
...
-T2_Inverse_Kinematics.exe:
 ...
 when considering a kinematic tree and not a chain, see doc.pdf) describing the problem are expressed 
 in a .txt file, see the examples provided for the formats.
...
-T3_Frame_Calibration.exe:
 ...
 Put a calibrator.stl object in the same position of the frame of interest  and then save the position
 of that .stl w.r.t. the frame you want to know the relative position.
 The corner of the calibrator is the only vertex having a cone of facets that are all mutually orthogonal. 
 The position of such a corner must be the same of the origin of the frame to calibrate;
 while the x axis must contain the longest edge connected to the corner; the y axis must contain the 
 medium size one and finally the z axis must contain the shortest.
 See Calibrator.png.
 It is possible to rescale the Calibrator, without varying the proportions.

-T4_Mechanism_Calibration.exe:
 For automatically calibrates a mechanism, i.e. compute the offsets of every rotating/traslating joint.
 A new .xml with the computed offsets is automatically written.
